                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg divide-y">
                    @foreach ($products as $product)
                        <div class="flex justify-between items-center p-4">
                            <div>
                                <div class="text-xs text-gray-500">{{ $product->shelf->getLong() }}</div>
                                <div class="text-gray-900 font-medium">{{ $product->name }}</div>
                            </div>

                            <div class="text-sm text-gray-700">{{ $product->default_price->toComptability(true) }}</div>
                        </div>
                    @endforeach
                </div>
            </div>
            <p class="text-center text-sm text-gray-700">
                {{ $products->count() }} enregistrement(s)
            </p>
        </div>
    </div>
</x-app-layout>
